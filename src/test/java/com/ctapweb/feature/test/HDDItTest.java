...
public class HDDItTest {
	...
	public void setUp() throws Exception {
		...
		DescriptorModifier.readXMLTypeDescriptorModifyImports ("src/main/resources/descriptor/type_system/feature_type/HDDType.xml", testResourcesFolder+"HDDTypeForUIMAFitTest.xml", locationsList);
		String sdSentenceLengthTypeDescr = new String(Files.readAllBytes(Paths.get(testResourcesFolder+"HDDTypeForUIMAFitTest.xml")));
	...
	public void HDDLemmaFeatureTest() throws Exception {
		...
		locationsListForTest.add("../../../../src/main/resources/descriptor/type_system/feature_type/HDDType.xml");
	
		File f = DescriptorModifier.readXMLAnnotatorDescriptorAddLanguageAddParamsFromHashModifyImports ("src/main/resources/descriptor/featureAE/HDD_Lemma_Feature.xml", testResourcesFolder+"HDD_Lemma_FeatureForUIMAFitTest.xml", paramsHashMap, locationsListForTest);
		...
		for(ComplexityFeatureBase annot : JCasUtil.select(jCas, ComplexityFeatureBase.class)){
			if(annot.getId() == 9001){
				assertEquals(0.8007083133124769, annot.getValue(), 0.0000001);
			}	
		}
	}
	...
	@Test
	public void HDDTokenFeatureTest() throws Exception {
		paramsHashMap = new HashMap <String, ArrayList <String>> ();		
		ArrayList dynamicStringArray = new ArrayList(2);
		String[] names = {"integer", "9001"};
		dynamicStringArray.addAll(Arrays.asList(names));
		...
		File f = DescriptorModifier.readXMLAnnotatorDescriptorAddLanguageAddParamsFromHashModifyImports ("src/main/resources/descriptor/featureAE/HDD_Token_Feature.xml", testResourcesFolder+"HDD_Token_FeatureForUIMAFitTest.xml", paramsHashMap, locationsListForTest);
		XMLInputSource xmlInputSource = new XMLInputSource(f);
		AnalysisEngineDescription aed = pars.parseAnalysisEngineDescription(xmlInputSource);
		...
		for(ComplexityFeatureBase annot : JCasUtil.select(jCas, ComplexityFeatureBase.class)){
			if(annot.getId() == 9001){
				assertEquals(0.9166874251270021, annot.getValue(), 0.0000001);
			}	
		}
	}
