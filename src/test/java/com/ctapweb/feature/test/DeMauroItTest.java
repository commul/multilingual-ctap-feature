...
public class DeMauroItTest {
	...
	public void setUp() throws Exception {
		...
		String sdSentenceLengthTypeDescr = new String(Files.readAllBytes(Paths.get(testResourcesFolder+"LexicalPercentageTypeForUIMAFitTest.xml")));
	...
	/*
	 * Checks that the percentage of lemmas of META-INF/cani.txt that is in the De Mauro basic dictionary.
	 */
	
	@Test
	public void DeMauroAllLemmasFeatureTest() throws Exception {
		...
		locationsListForTest.add("../../../../src/main/resources/descriptor/type_system/feature_type/LexicalPercentageType.xml");
		...
		for(ComplexityFeatureBase annot : JCasUtil.select(jCas, ComplexityFeatureBase.class)){
			if(annot.getId() == 9020){
				assertEquals(65.993265993266, annot.getValue(), 0.0000001);
			}	
		}
	}
	
	/*
	 * Checks that the percentage of unique lemmas of META-INF/cani.txt that is in the De Mauro basic dictionary.
	 */
	
	@Test
	public void DeMauroUniqueLemmasFeatureTest() throws Exception {
		paramsHashMap = new HashMap <String, ArrayList <String>> ();		
		ArrayList dynamicStringArray = new ArrayList(2);
		String[] names = {"integer", "9020"};
		dynamicStringArray.addAll(Arrays.asList(names));
		...
		for(ComplexityFeatureBase annot : JCasUtil.select(jCas, ComplexityFeatureBase.class)){
			if(annot.getId() == 9020){
				assertEquals(70.1219512195122, annot.getValue(), 0.0000001);
			}	
		}
	}
